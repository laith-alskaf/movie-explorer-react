...
const MovieDetailPage = () => {
  ...
  const fetchMovieDetail = useMovieDetailStore((s) => s.fetchMovieDetail);
  const resetDetail = useMovieDetailStore((s) => s.resetDetail);

  // حالة محلية للنافذة المنبثقة (لا تحتاج متجر لأنها خاصة بهذه الصفحة فقط)
  const [isModalOpen, setIsModalOpen] = useState(false);

  /**
   * 🧠 useEffect هنا يفعل شيئين:
   *   1. عند الدخول: يجلب تفاصيل الفيلم بناءً على الـ id من الرابط
  ...
  useEffect(() => {
    ...
    // Cleanup: عند مغادرة الصفحة أو تغيّر الـ id
    return () => resetDetail();
  }, [id, language]);
    ...
    <main className="min-h-screen bg-[#030014] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-5 py-8 md:py-14">
        ...
        >
          ...
          <span className="text-xs font-bold uppercase tracking-wider">{language === 'ar' ? 'رجوع' : 'BACK'}</span>
        </button>

        {/* الكرت الزجاجي الكبير الحاوي لكل التفاصيل */}
        <div className="w-full rounded-3xl bg-white/[0.01] border border-white/5 backdrop-blur-2xl p-6 md:p-10 shadow-2xl flex flex-col md:flex-row gap-8 lg:gap-12">

          {/* الجزء الأيسر: البوستر */}
          <div className="w-full md:w-72 lg:w-80 flex-shrink-0 mx-auto md:mx-0">
            <div className="rounded-2xl overflow-hidden border border-white/10 shadow-2xl shadow-[#AB8BFF]/5">
              <img
            ...
            <div className="flex flex-wrap gap-3.5 mt-2">
              {trailerVideo ? (
                ...
                >
                  ...
                  <span>العرض الدعائي غير متوفر</span>
                </button>
              )}

              <button className="px-6 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold text-xs uppercase tracking-wider hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer flex items-center gap-2">
                <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
            ...
            {topCast.length > 0 && (
              <div className="border-t border-white/5 pt-5">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-4">
                  {topCast.map((actor) => (
                    <div key={actor.id} className="flex flex-col items-center gap-2 text-center group">
                      ...
                      <span className="text-light-100 text-xs font-semibold leading-tight line-clamp-1">
                        {actor.name}
                      </span>
                    </div>
                  ))}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
              {similarMovies.map((sim) => (
                <Link
                  key={sim.id}
                  to={`/movie/${sim.id}`}
                  className="block group rounded-2xl overflow-hidden bg-[#0f0d23]/30 border border-white/5 hover:border-[#AB8BFF]/30 hover:shadow-[0_8px_20px_rgba(171,139,255,0.15)] hover:-translate-y-1 transition-all duration-300"
                >
